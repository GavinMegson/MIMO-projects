...
DataReceiver::DataReceiver(int N_THREAD, Config *cfg)
{
    ...
    srand (time(NULL));
    context = new DataReceiverContext[thread_num_];

}

DataReceiver::DataReceiver(int N_THREAD, Config *config, moodycamel::ConcurrentQueue<Event_data> * in_queue):
DataReceiver(N_THREAD, config)
{
...
std::vector<pthread_t> DataReceiver::startRecv(void** in_buffer, int** in_buffer_status, int in_buffer_frame_num, int in_buffer_length, int in_core_id)
{
    // check length
    ...
    for(int i = 0; i < thread_num_; i++)
    {
        ...
        context[i].tid = i;
        // start socket thread
        if(pthread_create( &recv_thread_, NULL, DataReceiver::loopRecv, (void *)(&context[i])) != 0)
        {
...
void* DataReceiver::loopRecv(void *in_context)
{
    // get the pointer of class & tid
    DataReceiver* obj_ptr = ((DataReceiverContext *)in_context)->ptr;
    Config *cfg = obj_ptr->config_;
    int tid = ((DataReceiverContext *)in_context)->tid;
    ...
    while(true)
    {
        ...
        for (int it = 0 ; it < nradio_cur_thread; it++) // FIXME: this must be threaded
        {
            ...
            // printf("Recv thread %d: receiving data for radio %d in buffer [%d,%d],[%d, %d]\n",tid, rid,(ushort)((ushort *)samp1-(ushort *)buffer),(ushort)((ushort *)samp2-(ushort *)buffer),*((short *)samp[0]),*((short *)samp[1]));
            ...
            *((int *)cur_ptr_buffer + 3) = ant_id;
#ifdef DEBUG2
            for (int i = 0 ; i < cfg->sampsPerSymbol; i++)
                *((int *)cur_ptr_buffer + 4 + i) = (ant_id > 0) ? i : cfg->sampsPerSymbol-i;
            printf("\nwriting vector value %d\n", (ant_id > 0) ? frame_id << 16 | frame_id : (65536-frame_id) << 16 | (65536-frame_id) );
#endif
            if (cfg->nChannels == 2)
            {
            ...
            cur_ptr_buffer = buffer + ((char*)cur_ptr_buffer - (char*)buffer + cfg->getPackageLength() * cfg->nChannels) % buffer_length;
            ...
            if (cfg->nChannels == 2)
            {
                ...
                cur_ptr_buffer2 = buffer2 + ((char*)cur_ptr_buffer2 - (char*)buffer2 + cfg->getPackageLength() * cfg->nChannels) % buffer_length;
                // push EVENT_PACKAGE_RECEIVED event into the queue
